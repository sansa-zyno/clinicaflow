...
class EditAppointmentSettings extends StatefulWidget {
  final CreateStaff createStaff;
  final StaffByIdModel staffByIdModel;
  const EditAppointmentSettings({super.key, required this.createStaff, required this.staffByIdModel});

  @override
  State<EditAppointmentSettings> createState() => _EditAppointmentSettingsState();
}

class _EditAppointmentSettingsState extends State<EditAppointmentSettings> with AppBarMixin {
  ...
  void initState() {
    super.initState();
    appointmentSlots = widget.staffByIdModel.availableTimeSlot;
  }
      ...
      body: BlocListener<StaffCubit, StaffState>(
        listener: (context, state) {
          if (state.state == StaffStates.staffCreated) {
            context.read<StaffDetailCubit>().fetchData(widget.staffByIdModel.id);
            Navigator.pop(context);
            Navigator.pop(context);
            Navigator.pop(context);
            Navigator.pop(context);
            Navigator.pop(context);
            showSnackbar("Staff details updated successfully", context);
          }
        },
                  ...
                  state.state == StaffStates.creatingStaff
                      ...
                      : GestureDetector(
                          onTap: () async {
                            ...
                            //dev.log(clinicId);
                            widget.createStaff.staffId = widget.staffByIdModel.staffId;
                            widget.createStaff.availableTimeSlot = appointmentSlots;
                            widget.createStaff.clientId = clinicId;
                            if (clinicId != '') {
                              context.read<StaffCubit>().updateStaff(widget.createStaff, context, widget.staffByIdModel.id);
                            }
                          },
                          child: Container(
                            height: 52,
