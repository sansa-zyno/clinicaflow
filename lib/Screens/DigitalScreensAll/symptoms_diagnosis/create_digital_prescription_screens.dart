...
class _CreateDigitalPrescriptionScreensState extends State<CreateDigitalPrescriptionScreens> with DeviceInfoMixin, UiInfoMixin {
  ...
  final List<Symptom> associatedSymptoms = [
    Symptom(type: 'Sx', name: "Shallow breathing"),
    Symptom(type: 'Sx', name: "Fever"),
    Symptom(type: 'Sx', name: "Sweating & chills"),
    Symptom(type: 'Sx', name: "Headache"),
    Symptom(type: 'Sx', name: "Muscle pain")
  ];

  final List<Symptom> differentialDiagnosis = [
    Symptom(type: 'Dx', name: "Pneumonia"),
    Symptom(type: 'Dx', name: "Malaria"),
    Symptom(type: 'Dx', name: "Typhoid"),
    Symptom(type: 'Dx', name: "Common cold"),
    Symptom(type: 'Dx', name: "Influenza")
  ];
  ...
  Widget build(BuildContext context) {
    return BlocListener<SymptomsAndDiagnosisCubit, SymptomsAndDiagnosisState>(
      listener: (context, state) {
        if (state.state == SymptomsAndDiagnosisStates.searchingSymptomAndPredictionForddxFailed) {
      ...
      child: Scaffold(
          ...
          //endDrawer: const DigitalPrescriptionEndDrawer(),
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        buildSection(title: "Associated Symptoms", color: const Color(0xff1B9C85), symptoms: associatedSymptoms ?? []),
                        ...
                        buildSection(title: "Differential Diagnosis", color: const Color(0xff1B9C85), symptoms: differentialDiagnosis ?? []),
                        const Divider(
                          color: AppColors.lightGrey,
                        ),
                        Column(
                      ...
                      child: MyElevatedButton(
                          ...
                          onPressed: () {
                            final Map<String, dynamic> body = {
                              "input_symptoms": selectedSymptoms.map((e) => e.name).toList(),
                              "input_diagnoses": [],
                              "n_diseases": selectedSymptoms.length,
                              "n_symptoms": selectedSymptoms.length,
                              "min_symptoms": selectedSymptoms.length
                            };

                            log("MAKE REQUEST WITH: $body");

                            context.read<SymptomsAndDiagnosisCubit>().searchSymptomAndPredictionForddx(body);
                          })),
