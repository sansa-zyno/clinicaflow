...
class _NotificationScreenState extends State<NotificationScreen> {
  String calcTimesAgo(DateTime dt) {
    ...
    if (dur.inSeconds < 60) {
      ...
      String date = "${dte.year} ${dte.month} ${dte.day}" == "${dateNow.year} ${dateNow.month} ${(dateNow.day) - 1}"
          ? "Yesterday"
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocBuilder<NotificationCubit, NotificationState>(builder: (context, state) {
        if (state.state == NotificationStates.fetchingNotifications) {
          return const Center(child: CircularProgressIndicator());
        } else if (state.state == NotificationStates.fetchingNotificationsFailed) {
          return const Center(child: CircularProgressIndicator());
                      ...
                      itemBuilder: (context, index) {
                        return Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            boxShadow: [
                              BoxShadow(
                                color: Colors.white.withOpacity(0.05), // Transparent white color
                            ...
                            children: [
                              Text(
                                ...
                                style: TextStyle(
                                  ...
                                  fontSize: 16.0,
                                  color: state.notificationList![index]['header'] == 'Appointment'
                                      ? const Color(0xFF5351C7)
                                      : state.notificationList![index]['header'] == 'Payment'
                              ...
                              Row(
                                children: [
                                  Expanded(
                                    child: Text(
                                      '${state.notificationList![index]['notificationMessage']}'.trimLeft(),
                                      style: const TextStyle(
                                        fontFamily: 'Montserrat',
                              ...
                              Text(
                                calcTimesAgo(DateTime.parse(state.notificationList![index]['showTime'])),
                                style: const TextStyle(
                                  fontFamily: 'Poppins',
                                  fontWeight: FontWeight.w500,
                                  fontSize: 12.0,
