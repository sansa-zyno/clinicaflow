...
import 'package:clinica_flow/core/constants/app_text.dart';
...
import 'package:clinica_flow/core/constants/app_constants.dart';
...
import 'package:clinica_flow/core/utils/mixins/time_parser_mixin.dart';
...
import 'package:clinica_flow/shared/widgets/components/build_section.dart';
import 'package:clinica_flow/shared/widgets/components/dual_action_bottom_nav.dart';
import 'package:clinica_flow/shared/widgets/components/my_search_bar.dart';
import 'package:clinica_flow/shared/widgets/components/vitals_and_past_history_end_drawer.dart';
...
class DigitalPrecriptionScreen extends StatefulWidget {
  ...
  const DigitalPrecriptionScreen(
      {super.key, required this.appointment, this.savedDrugPrescription});
...
class _DigitalPrecriptionScreenState extends State<DigitalPrecriptionScreen>
    with DeviceInfoMixin, TimeParserMixin, UiInfoMixin {
  ...
  void initState() {
    ...
    otherInstructions.text =
        widget.savedDrugPrescription?['privateNotes'] ?? '';
    followUpDate = () {
      if (widget.savedDrugPrescription?['followUpDate'] == null) {
        return {'None': null};
      } else {
        if (DateTime.parse(widget.savedDrugPrescription?['followUpDate'])
                .difference(DateTime.now())
                .inDays <=
            3) {
          return {
            'After 3 days':
                DateTime.parse(widget.savedDrugPrescription?['followUpDate'])
          };
        } else if (DateTime.parse(widget.savedDrugPrescription?['followUpDate'])
                .difference(DateTime.now())
                .inDays <=
            7) {
          return {
    ...
    String? start = widget.savedDrugPrescription?['followUpTimeSlot']
        .toString()
        .split('-')[0];
    String? finish = widget.savedDrugPrescription?['followUpTimeSlot']
        .toString()
        .split('-')[1];

    if (start != null) {
      if (start.toLowerCase().contains('am') ||
          start.toLowerCase().contains('pm')) {
        timeSlot = timeSlot.copyWith(
            start: TimeOfDay.fromDateTime(
                DateFormat("h:mm a").parse(start.trimLeft())));
      } else {
        timeSlot = timeSlot.copyWith(
            start: TimeOfDay.fromDateTime(
                DateFormat("hh:mm").parse(start.trimLeft())));
      }
    }
    if (finish != null) {
      if (finish.toLowerCase().contains('am') ||
          finish.toLowerCase().contains('pm')) {
        timeSlot = timeSlot.copyWith(
            finish: TimeOfDay.fromDateTime(
                DateFormat("h:mm a").parse(finish.trimLeft())));
      } else {
        timeSlot = timeSlot.copyWith(
            finish: TimeOfDay.fromDateTime(
                DateFormat("hh:mm").parse(finish.trimLeft())));
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        endDrawer:
            VitalsAndPastHistoryEndDrawer(appointment: widget.appointment),
        body: BlocListener<DrugPrescriptionCubit, DrugPrescriptionState>(
          listener: (context, state) {
            if (state.state == DrugPrescriptionStates.drugPrescriptionPosted &&
                !hasNavigated) {
              dev.log(state.state.toString());
              hasNavigated = true;
              showSnackbar("Drug prescription saved successfully", context);
              context.read<DrugPrescriptionCubit>().getSavedDrugPrescription(
                  appointmentId: widget.appointment.id!);
              context
                  .read<PrescriptionReportCubit>()
                  .getPrescriptionReport(appointmentId: widget.appointment.id!);
              context.pop();
            }
          },
          child: BlocBuilder<DrugPrescriptionCubit, DrugPrescriptionState>(
              ...
              slivers: [
                ...
                isTyping
                    ...
                    : selectedDrugs.isNotEmpty == true
                        ? SliverToBoxAdapter(
                            child:
                                buildAddDrugsView()) // view when the doctor has saved atleast one drug for this patient
        ...
        bottomNavigationBar:
            BlocBuilder<DrugPrescriptionCubit, DrugPrescriptionState>(
                builder: (context, state) {
            ...
            return DualActionBottomNav(
                ...
                onFocusedPressed: () {
                  if (selectedDrugs.isNotEmpty) {
                    dev.log(
                        "${timeSlot.start?.format(context) ?? ''} - ${timeSlot.finish?.format(context)}");
  ...
  Widget buildAddDrugsView() {
    return BlocBuilder<DrugPrescriptionCubit, DrugPrescriptionState>(
        builder: (context, state) {
      return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                children: [
                  ...
                  MyIconContainer(
                      ...
                      icon: const Icon(Icons.close,
                          color: AppColors.darkBlueViolet, size: 18))
                ],
        ...
        TitledTextField(
            ...
            hintText: "e.g Some other instruction",
            maxLines: 8,
            minLines: 8),
        ...
        Wrap(
            ...
            children: List<Widget>.generate(
                AppConstants.followUpDate.keys.length, (index) {
    ...
    return BlocBuilder<DrugPrescriptionCubit, DrugPrescriptionState>(
      ...
      if (state.state == DrugPrescriptionStates.searchingForDrugs) {
        return SliverToBoxAdapter(child: Container());
          ...
          DrugPrescriptionStates.searchingForDrugsFailed) {
        return SliverToBoxAdapter(child: Container());
      } else {
  ...
  Widget buildNotTypingView(DrugPrescriptionState state) {
    if (state.state == DrugPrescriptionStates.fetchingFrequentlySearchedDrugs) {
      return SliverToBoxAdapter(
          child:
              AppConstants.buildPlaceHolder(title: 'Frequently searched drugs')
                  .pSymmetric());
    } else if (state.state ==
        DrugPrescriptionStates.frequentlySearchedDrugsFailed) {
      return SliverToBoxAdapter(
          child:
              AppConstants.buildPlaceHolder(title: 'Frequently searched drugs')
                  .pSymmetric());
        ...
        child: Column(
          ...
          const SizedBox(
            ...
            BuildSection(
              ...
              children: List<Widget>.generate(
                  state.frequentlySearchedDrugs?.length ?? 0, (index) {
                      ...
                      children: [
                        ...
                        Wrap(
                          ...
                          children: List<Widget>.generate(
                            ...
                            return SelectableContainer(
                              selected: drug.dosageFrequency == freq ||
                                  (freq == "Other" && other),
                              onTap: () {
                                setState(() {
                                  if (freq == "Other") {
                        ...
                        if (other)
                          CustomTextField(
                            controller: TextEditingController(
                                text: drug.dosageFrequency?.toString() ?? ''),
                            hintText: "Other",
                            usePadding: false,
                            fillColor: AppColors.whiteSmoke,
                            ...
                            children: List<Widget>.generate(
                              ...
                              return SelectableContainer(
                                ...
                                onTap: () {
                                  setState(() {
                                    drug =
                                        drug.copyWith(dosageTime: dosageTime);
                                  });
                                },
                                title: Text(
                                  dosageTime,
                        ...
                        CustomTextField(
                                controller: TextEditingController(
                                    text: drug.duration != null
                                        ? "For ${drug.duration!['value']} ${drug.duration!['unit']}"
                                        : ''),
                                usePadding: false,
                                readOnly: true,
                                onTap: () async {
                                                  ...
                                                  children:
                                                      List<Widget>.generate(
                                                          AppConstants
                                                              ...
                                                              .keys
                                                              .length, (index) {
                                                    final key = AppConstants
                                                        .drugDuration.keys
                                                        .elementAt(index);
                                                    final value = AppConstants
                                                        .drugDuration[key];
                  ...
                  bottomNavigationBar: DualActionBottomNav(
                      ...
                      onFocusedPressed: () {
                        ...
                        if (drug.name != "" &&
                          ...
                          showSnackMessage(
                              context, 'You need to fill all the drug details');
                        }
                      }),
                ),
              ),
