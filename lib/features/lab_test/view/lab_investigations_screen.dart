...
class LabInvestigationsScreen extends StatefulWidget {
  ...
  const LabInvestigationsScreen(
      {Key? key, required this.appointment, this.selectedTests})
      : super(key: key);
...
class _LabInvestigationsScreenState extends State<LabInvestigationsScreen> {
  ...
  TextStyle get boldTitleUrbanist => GoogleFonts.urbanist(
  ...
  TextStyle get hintStyle => GoogleFonts.roboto(
      textStyle: const TextStyle(
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 16),
                      child: Text("Notes", style: boldTitleUrbanist)
                          .pOnly(bottom: 4),
                    ),
                    ...
                    Padding(
                      ...
                      child: CustomTextField(
                          ...
                          hintStyle: hintStyle.copyWith(
                              fontWeight: FontWeight.w400, height: 17.16 / 13),
                          minLines: 5,
                          maxLines: 10),
                    )
                  ],
        ...
        child: BlocListener<LabTestCubit, LabTestState>(
          listener: (context, state) {
            if (state.state == LabTestStates.labTestsPosted && !hasNavigated) {
              ...
              context
                  .read<LabTestCubit>()
                  .getSavedLabTests(appointmentId: widget.appointment.id!);
              context
                ...
                return DualActionBottomNav(
                    ...
                    onFocusedPressed: () {
                      ...
                      context.read<LabTestCubit>().postLabTest(
                          ...
                          labTest:
                              selectedTests.map((e) => e.toMap()).toList());
                    });
              }
            })
          : null,
      ...
      builder: (context) => Positioned(
        ...
        child: CompositedTransformFollower(
          ...
          child: Material(
              elevation: 1.0,
              child: Container(
                  height: 600, child: SingleChildScrollView(child: widget))),
        ),
      ),
    );
  }
  ...
  Widget buildTypingView() {
    return BlocBuilder<LabTestCubit, LabTestState>(builder: (context, state) {
      if (state.state == LabTestStates.searchingForTests) {
        ...
        return Column(
            children: List<Widget>.generate(state.availableTests?.length ?? 0,
                (index) {
  ...
  Column buildNotTypingView(LabTestState state) {
    return Column(
      ...
      children: [
        ...
        if (state.state == LabTestStates.fetchingFrequentlySearchedTests)
          AppConstants.buildPlaceHolder(title: 'Frequently Searched Tests'),
        if (state.state == LabTestStates.frequentlySearchedTestsFailed)
          AppConstants.buildPlaceHolder(title: 'Frequently Searched Tests'),
  ...
  SelectableContainer buildActionButton(
