...
import 'package:clinica_flow/core/constants/typography.dart';
...
class _PastMedicalHistoryScreenState extends State<PastMedicalHistoryScreen>
  ...
  void initState() {
    ...
    if (widget.pastHistory != null && widget.pastHistory!.isNotEmpty) {
      pastHistory = List.generate(
          widget.pastHistory!.length, (index) => widget.pastHistory![index]);
    } else {
      pastHistory = List.filled(2, HistoryItem.empty(),
          growable: true); // initial two items according to the figma UI
    }
    if (widget.familyHistory != null && widget.familyHistory!.isNotEmpty) {
      familyHistory = List.generate(widget.familyHistory!.length,
          (index) => widget.familyHistory![index]);
    } else {
      familyHistory = List.filled(2, HistoryItem.empty(),
          growable: true); // initial two items according to the figma UI
    }
    if (widget.pastProcedureHistory != null &&
        widget.pastProcedureHistory!.isNotEmpty) {
      pastProcedures = List.generate(widget.pastProcedureHistory!.length,
          (index) => widget.pastProcedureHistory![index]);
    } else {
      pastProcedures = List.filled(2, HistoryItem.empty(),
          growable: true); // initial two items according to the figma UI
    }
    if (widget.allergies != null && widget.allergies!.isNotEmpty) {
      allergies = List.generate(
          widget.allergies!.length, (index) => widget.allergies![index]);
    } else {
      allergies = List.filled(2, HistoryItem.empty(),
          growable: true); // initial two items according to the figma UI
    }
    if (widget.medication != null && widget.medication!.isNotEmpty) {
      medicationHistory = List.generate(
          widget.medication!.length, (index) => widget.medication![index]);
    } else {
      medicationHistory = List.filled(2, HistoryItem.empty(),
          growable: true); // initial two items according to the figma UI
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: buildAppBar(context,
          title: "Digital Prescription", backgroundColor: AppColors.whitColor),
      body: BlocListener<PastMedicalHistoryCubit, PastMedicalHistoryState>(
          listener: (context, state) {
            if (state.state ==
                    PastMedicalHistoryStates.pastMedicalHistoryPosted &&
                !hasNavigated) {
              hasNavigated = true;
              showSnackMessage(
                  context, 'Past Medical History saved successfully.');
              context.read<PastMedicalHistoryCubit>().getPastMedicalHistory(
              ...
              context
                  ...
                  .getAppointmentById(id: widget.appointment.id!);
      ...
      bottomNavigationBar:
          BlocBuilder<PastMedicalHistoryCubit, PastMedicalHistoryState>(
            ...
            onFocusedPressed: () {
              ...
              context.read<PastMedicalHistoryCubit>().postPastMedicalHistory(
                  ...
                  pastProcedureHistory:
                      pastProcedures.map((e) => e.toMap()).toList());
            },
          );
        }
      }),
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          MyIconContainer(
              ...
              icon: const Icon(Icons.close,
                  color: AppColors.darkBlueViolet, size: 14))
        ]));
  }
}
      ...
      children: [
        ...
        GestureDetector(
          ...
          child: SectionText(
            ...
            textStyle:
                AppTypography.CTA_1_SB.copyWith(color: AppColors.darkTeal),
            underlineWidth: 40,
...
class _HistoryItemWidgetState extends State<HistoryItemWidget> {
                        ...
                        child: CustomTextField(
                            ...
                            onChanged: (value) {
                              if (value.length == 4) {
                                ...
                                item =
                                    item.copyWith(year: noOfYears.toString());
                              } else {
      ...
      builder: (context) => Positioned(
        ...
        child: CompositedTransformFollower(
          ...
          child: Material(
              elevation: 1.0,
              child: Container(
                  height: 325, child: SingleChildScrollView(child: widget))),
        ),
      ),
    );
  }*/

  Widget buildAllergySearchResults(
      TextEditingController controller, HistoryItem item, int itemIndex) {
    return BlocBuilder<AllergyCubit, AllergyState>(builder: (context, state) {
      if (state.state == AllergyStates.searchingForAllergies) {
        ...
        return Column(
            children:
                List<Widget>.generate(state.allergies?.length ?? 0, (index) {
  ...
  Widget buildMedicationsSearchResults(
      TextEditingController controller, HistoryItem item, int itemIndex) {
