...
import '../viewmodel/appointment_cubit.dart';
...
class _ScheduleAppointmentScreenState extends State<ScheduleAppointmentScreen> {
  ...
  List<PatientModel> duplicatePatients = [];
  PatientModel? selectedPatientOption;
  ...
  fetchPatients() async {
    allPatients = await PatientService().fetchPatients();
    setState(() {});
  }
    ...
    if (_isFormComplete()) {
      ...
      Map map = {
        ...
        'patientId': (patientId == null ||
                patientId ==
                    '-1') //New patient with new number or New patient on Existing number
            ? ''
            : selectedPatientOption?.id, //Existing patient with Existing number
        'doctorId': selectedDoctor!['_id'],
        'doctorName':
            '${selectedDoctor!['firstName']} ${selectedDoctor!['lastName']}',
        'clinicPatientId': (patientId == null ||
                patientId ==
                    '-1') //New patient with new number or New patient on Existing number
            ? ''
            : selectedPatientOption?.id //Existing patient with Existing number
      ...
      if (appointmentCubit.state.state ==
        ...
        if (appointmentCubit.state.state ==
          ...
          Appointment appointmentDetails =
              appointmentCubit.state.appointmentDetails!;
          mobileNumberController.text = '';
          firstNameController.text = '';
          ageController.text = '';
          ...
          context.pushNamed(AppRoutes.appointmentSuccess.name,
              extra: appointmentDetails);
        } else if (appointmentCubit.state.state ==
            AppointmentStates.fetchingAppointmentByIdFailed) {
          showSnackbar(
              'An error occured while scheduling appointment', context);
        }
      } else if (appointmentCubit.state.state ==
            ...
            children: [
                ...
                Container(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      PatientModel item = PatientModel();
                      ...
                      return GestureDetector(
                        onTap: () async {
                          if (index < duplicatePatients.length) {
                            ...
                            PatientModel patientDetails = await PatientService()
                            ...
                            setState(() {
                              selectedPatientOption = PatientModel(
                                  id: '-1',
                                  firstName: 'Add New Patient',
                                  lastName: '');
                              showDuplicateNumberMessage = false;
              ...
              BlocBuilder<AppointmentCubit, AppointmentState>(
                ...
                return data.state == AppointmentStates.creatingAppointments ||
                        data.state == AppointmentStates.fetchingAppointmentById
                    ? const Center(
