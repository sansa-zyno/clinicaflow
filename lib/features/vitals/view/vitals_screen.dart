...
import 'package:clinica_flow/shared/widgets/build_vital_item_widget.dart';
...
class VitalsScreen extends StatefulWidget {
  ...
  final Vital? vitals;
  const VitalsScreen({Key? key, required this.appointment, this.vitals})
      : super(key: key);
    ...
    if (widget.vitals != null && widget.vitals!.id != null) {
      ...
      if (vital.personalHistories == null) {
        ...
        if (personalHistories.isNotEmpty) {
          personalHistoryControllers = personalHistories
              .map((e) => TextEditingController(text: e.activity))
              .toList();
          personalHistoryLayerLinks =
              personalHistories.map((e) => LayerLink()).toList();
        } else {
          personalHistories = {
            PersonalHistory(activity: 'x'),
            PersonalHistory(activity: 'y'),
      ...
      vital = Vital(
          bloodPressure: BloodPressure(), personalHistories: personalHistories);
    }
  }
  ...
  void saveHistory(PersonalHistory newHistory) {
    setState(() {
      if (vital.personalHistories!.contains(newHistory)) {
        ...
        personalHistories = vital.personalHistories!
            .map((history) =>
                history.activity == newHistory.activity ? newHistory : history)
            .toSet();
        vital = vital.copyWith(personalHistories: personalHistories);
      } else {
        log("ADD NEW HISTORY");
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                AnimatedBuilder(
                  animation: Listenable.merge([_height, _weight]),
                  builder: (context, _) {
                    double bmi = 0;
                    String status = "N/A";
                    double h = double.tryParse(_height.text) ?? 0;
                      ...
                      children: [
                        ...
                        RichText(
                          text: TextSpan(
                            ...
                            children: [
                              const TextSpan(text: 'This is considered '),
                              TextSpan(
                                text: status,
                                style: const TextStyle(
                                    fontWeight: FontWeight.w700),
                      ...
                      children: [
                        GestureDetector(
                          onTap: () {
                            ...
                            if (personalHistories
                                    .elementAt(personalHistories.length - 1)
                                    .activity !=
                                '') {
                ...
                Column(
                  children: List<Widget>.generate(
                      personalHistoryControllers.length, (index) {
                    return Padding(
                      ...
                      child: Row(
                        children: [
                          Expanded(
                            child: CompositedTransformTarget(
                              ...
                              child: CustomTextField(
                                ...
                                onChanged: (value) {
                                  List<PersonalHistory> newList =
                                      personalHistories.toList();
                                  PersonalHistory history = personalHistories
                                      .elementAt(index)
                                      .copyWith(activity: value);
                                  newList[index] = history;
                                  personalHistories = newList.toSet();
                                  vital = vital.copyWith(
                          ...
                          InkWell(
                            onTap: () {
                              final List<PersonalHistory> match =
                                  personalHistories
                                      .where((element) =>
                                          element.activity ==
                                          personalHistoryControllers[index]
                                              .text)
                                      .toList();
                              if (match.isNotEmpty) {
                            ...
                            child: CircleAvatar(
                                radius: 12,
                                backgroundColor: personalHistories.isNotEmpty &&
                                        (personalHistories
                          ...
                          InkWell(
                            onTap: () {
                              if (personalHistoryControllers.length > 2) {
                                personalHistories.removeWhere((element) =>
                                    element.activity ==
                                    personalHistoryControllers[index].text);
          ...
          return Row(children: [
            ...
            Expanded(
              ...
              onPressed: () {
                ...
                hasNavigated = false;
                context.read<VitalsCubit>().postVitals(
                    patientId: widget.appointment.patientId!,
                    appointmentId: widget.appointment.id!,
                    map: vital.toMap());
              },
