...
import 'package:clinica_flow/features/symptoms_diagnosis/view/privacy_notes_sheet.dart';
...
class _CreateDigitalPrescriptionScreensState
    extends State<CreateDigitalPrescriptionScreens> with UiInfoMixin {
  ...
  getIsFirstTime() async {
    isFirstTime =
        await SharedPrefService.getFirstTimeOnPrescriptionScreen() ?? true;
    setState(() {});
  }
  ...
  void initState() {
    ...
    if (widget.selectedSymptoms != null &&
      ...
      symptomsControllers = selectedSymptoms
          .map((e) => TextEditingController(text: e.name))
          .toList();
      symptomsLayerLinks = selectedSymptoms.map((e) => LayerLink()).toList();
      symptomsDurationControllers = selectedSymptoms
          .map((e) => TextEditingController(
              text: '${e.timePeriod ?? ''} ${e.timeUnit ?? ''}'))
          .toList();
      symptomsDurationsLayerLinks =
          selectedSymptoms.map((e) => LayerLink()).toList();
    }
    if (widget.selectedDiagnosis != null &&
        widget.selectedDiagnosis!.isNotEmpty) {
      selectedDiagnosis = widget.selectedDiagnosis!;
      diagnosisControllers = selectedDiagnosis
          .map((e) => TextEditingController(text: e.name))
          .toList();
  ...
  Widget build(BuildContext context) {
    return BlocListener<SymptomsAndDiagnosisCubit, SymptomsAndDiagnosisState>(
      listener: (context, state) {
        if (state.state ==
            SymptomsAndDiagnosisStates
                .fetchingSymptomAndPredictionForddxFailed) {
          showMessage(
              context, "Error", state.error?.content ?? "An error occured");
        } else if (state.state ==
                SymptomsAndDiagnosisStates.symptomsAndDiagnosisPosted &&
            !hasNavigated) {
          dev.log(state.state.toString());
          hasNavigated = true;
          showSnackbar("Symptoms and Diagnosis saved successfully", context);
          context
              .read<SymptomsAndDiagnosisCubit>()
              .getSavedSymptomsAndDiagnosis(
      ...
      child: Scaffold(
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        BlocBuilder<SymptomsAndDiagnosisCubit,
                                SymptomsAndDiagnosisState>(
                            builder: (context, state) {
                          if (state.state ==
                              SymptomsAndDiagnosisStates
                                  .fetchingFrequentlySearchedSymptoms) {
                            return AppConstants.buildPlaceHolder(
                                        ...
                                        child: Row(
                                          children: [
                                                    ...
                                                    child: SizedBox(
                                                      ...
                                                      child: Column(
                                                        ...
                                                        children: [
                                                          InkWell(
                                                            onTap: () {
                                                              if (symptomsDurationControllers[
                                                                      index]
                                                                  .text
                                                                  .isNotEmpty) {
                                                                List<String>
                                            ...
                                            InkWell(
                                              ...
                                              child: CircleAvatar(
                                                  radius: 12,
                                                  backgroundColor: selectedSymptoms
                                                              .isNotEmpty &&
                                                          (selectedSymptoms
                                                                  .elementAtOrNull(
                                                                      index)
                              ...
                              children: [
                                ...
                                InkWell(
                                  onTap: () {
                                    ...
                                    if (isFirstTime) {
                                      SharedPrefService
                                          .setFirstTimeOnPrescriptionScreen(
                                              false);
                                      getIsFirstTime();
                                        ...
                                        child: Row(
                                          children: [
                                            ...
                                            InkWell(
                                              ...
                                              child: CircleAvatar(
                                                  radius: 12,
                                                  backgroundColor: selectedDiagnosis
                                                              .isNotEmpty &&
                                                          (selectedDiagnosis
                                                                  .elementAtOrNull(
                                                                      index)
                    ...
                    child: BackdropFilter(
                      ...
                      child: Container(
                        color: Colors.black.withOpacity(
                            0.5), // Optional: Add semi-transparent overlay
                      ),
                    ),
                  ),
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              margin: const EdgeInsets.only(
                                  top:
                                      18), // Adjust margin for close button space
          ...
          bottomNavigationBar: selectedSymptoms.isNotEmpty ||
                  selectedDiagnosis.isNotEmpty
              ? BlocBuilder<SymptomsAndDiagnosisCubit,
                  SymptomsAndDiagnosisState>(builder: (context, state) {
                  if (state.state ==
                      SymptomsAndDiagnosisStates.postingSymptomsAndDiagnosis) {
                    ...
                    return Row(children: [
                      ...
                      Expanded(
                          child: MyElevatedButton(
                              ...
                              height: 58,
                              textStyle: const TextStyle(
                                  color: AppColors.eerieBlack, fontSize: 15),
  ...
  void _showOverlay(BuildContext context, Widget widget, int index,
      LayerLink layerLink, bool isDuration) {
  ...
  OverlayEntry _createOverlayEntry(
      Widget widget, int index, LayerLink layerLink, bool isDuration) {
    return OverlayEntry(
          ...
          (index) => InkWell(
                onTap: () {
                  ...
                  Symptom updated = selectedSymptoms.elementAt(idx).copyWith(
                        timePeriod:
                            int.parse(durationController.text.split(' ')[0]),
  ...
  Widget buildSectionFrequentlySearchedsymptoms(
      {required List<Symptom> symptoms, Color? color}) {
          ...
          InkWell(
            onTap: () {
              BlocProvider.of<SymptomsAndDiagnosisCubit>(context)
                  .fetchFrequentlySearchedSymptoms();
            },
            child: const Icon(
              Icons.refresh,
        ...
        children: [
          ...List<Widget>.generate(symptoms.length, (index) {
            ...
            return GestureDetector(
              onTap: () {
                TextEditingController controller = symptomsControllers
                    .firstWhere((element) => element.text == "");
  ...
  Widget buildSectionDifferentialDiagnosis(
      {required List<Diagnosis> diagnosis, Color? color}) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(
        children: [
          Text(
          ...
          InkWell(
            onTap: () {
              if (selectedSymptoms.isNotEmpty) {
                BlocProvider.of<SymptomsAndDiagnosisCubit>(context)
                    .ddxPredictions(
                        selectedSymptoms.map((e) => e.name).toList(),
                        selectedDiagnosis.map((e) => e.name).toList());
              }
            },
            child: const Icon(
              Icons.refresh,
      ...
      Wrap(
        ...
        children: [
          ...List<Widget>.generate(diagnosis.length, (index) {
            Symptom symptom =
                Symptom(name: diagnosis[index].diseaseName, type: 'Dx');

            return GestureDetector(
              onTap: () {
                TextEditingController controller = diagnosisControllers
                    .firstWhere((element) => element.text == "");
                controller.text = symptom.name;
                selectedDiagnosis.add(symptom);
              },
              child: Container(
                  ...
                  decoration: BoxDecoration(
                      color: Color(int.parse(diagnosis[index].color))),
                  child: Text(
                    symptom.name.capitalize,
                  )),
            );
  ...
  Widget buildSectionAssociatedSymptoms(
      {required List<Diagnosis> diagnosis, Color? color}) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(
        children: [
  ...
  void saveSymptom(Symptom newSymptom) {
    setState(() {
      if (selectedSymptoms.contains(newSymptom)) {
        ...
        selectedSymptoms = selectedSymptoms
            .map((symptom) =>
                symptom.name == newSymptom.name ? newSymptom : symptom)
            .toSet();
      } else {
        log("ADD NEW SYMPTOM");
