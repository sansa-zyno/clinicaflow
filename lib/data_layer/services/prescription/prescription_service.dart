...
class PrescriptionService {
  ...
  Future<PrescriptionReport?> getPrescriptionReport({required String appointmentId}) async {
    await fetchToken();
    try {
      final response = await HttpService.get(ApiEndPoint.getPrescriptionReport(appointmentId: appointmentId, clinicId: clinicId), token);
      if (response.statusCode == 200) {
