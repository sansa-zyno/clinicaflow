...
class LabTestCubit extends Cubit<LabTestState> {
  ...
  void fetchFrequentlySearchedTests() async {
    emit(state.copyWith(state: LabTestStates.fetchingFrequentlySearchedTests, savedTests: state.savedTests));

    try {
      List<LabTest> labTests = await service.getFrequentlySearchedTests();

      emit(state.copyWith(state: LabTestStates.frequentlySearchedTestsFetched, frequentlySearchedTests: labTests, savedTests: state.savedTests));
    } catch (error) {
      log('Failed to load tests: $error');

      emit(state.copyWith(state: LabTestStates.frequentlySearchedTestsFailed, savedTests: state.savedTests));
    }
  }

  void searchTests(String query) {
    emit(state.copyWith(state: LabTestStates.searchingForTests, savedTests: state.savedTests));
    //TODO: ADD LOGIC HERE
    try {
      emit(state.copyWith(
      ...
      emit(state.copyWith(state: LabTestStates.searchingForTestsFailed, savedTests: state.savedTests));
    }
  }

  postLabTest({required String patientId, required String appointmentId, required List<Map<String, dynamic>> labTest}) async {
    emit(state.copyWith(state: LabTestStates.postingLabTests, savedTests: state.savedTests));
    try {
      String message = await service.postLabTest(patientId: patientId, appointmentId: appointmentId, labTests: labTest);
      emit(state.copyWith(state: LabTestStates.labTestsPosted, savedTests: state.savedTests));
    } catch (error) {
      log('Failed to load ddxPredictions: $error');

      emit(state.copyWith(state: LabTestStates.postingLabTestsFailed, savedTests: state.savedTests));
    }
  }
