...
class SymptomsAndDiagnosisCubit extends Cubit<SymptomsAndDiagnosisState> {
  ...
  ddxPredictions(List<String> selectedSymptoms, List<String> selectedDiagnosis) async {
    emit(state.copyWith(
        state: SymptomsAndDiagnosisStates.fetchingSymptomAndPredictionForddx,
        savedSymptoms: state.savedSymptoms,
        savedDiagnosis: state.savedDiagnosis));
    try {
      List<Diagnosis> diagnosis = await service.ddxPredictions(selectedSymptoms, selectedDiagnosis);
      emit(state.copyWith(
      ...
      emit(state.copyWith(
          state: SymptomsAndDiagnosisStates.fetchingSymptomAndPredictionForddxFailed,
          savedSymptoms: state.savedSymptoms,
          savedDiagnosis: state.savedDiagnosis));
    }
  }
  ...
  postSymptomsAndDiagnosis({
    ...
    emit(state.copyWith(
        state: SymptomsAndDiagnosisStates.postingSymptomsAndDiagnosis, savedSymptoms: state.savedSymptoms, savedDiagnosis: state.savedDiagnosis));
    try {
      String message =
