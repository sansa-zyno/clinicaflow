...
class PatientRecordsCubit extends Cubit<PatientRecordsState> {
  ...
  Future<void> deletePatient(String id) async {
    try {
      ...
      final List<PatientOverviewModel> newPatients =
          state.patients != null ? [...state.patients!] : [];
      newPatients.removeWhere((patient) => patient.sId == id);

      emit(state.copyWith(
          patients: newPatients, state: PatientRecordsStates.patientDeleted));
    } catch (error) {
  ...
  Future<void> postPatient(PatientCreate patientCreate) async {
    ...
    try {
      patientCreate = await service.postPatient(patientCreate);
  ...
  Future<void> editPatient(PatientCreate patientCreate, String id) async {
    emit(state.copyWith(state: PatientRecordsStates.postingPatient));
    try {
      patientCreate = await service.editPatient(patientCreate);
      final List<PatientOverviewModel> newPatients =
          state.patients != null ? [...state.patients!] : [];
      newPatients.removeWhere((patient) => patient.sId == id);

      emit(state.copyWith(
          patients: newPatients, state: PatientRecordsStates.patientPosted));
    } catch (error) {
      final errorMessage = error.toString();
