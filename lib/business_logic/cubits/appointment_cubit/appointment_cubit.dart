...
class AppointmentCubit extends Cubit<AppointmentState> {
  ...
  Future<void> createAppointment(Map map) async {
    ...
    try {
      ...
      emit(state.copyWith(state: AppointmentStates.appointmentsCreated, id: id));
    } catch (error) {
  ...
  reScheduleAppointment({required String id, required String appointmentDate, required String timeSlot}) async {
    emit(state.copyWith(state: AppointmentStates.reschedulingAppointment));
    try {
      String? message = await service.reScheduleAppointment(id: id, appointmentDate: appointmentDate, timeSlot: timeSlot);
      emit(state.copyWith(state: AppointmentStates.appointmentRescheduled));
    } catch (error) {
      log('Failed to reschedule appointment: $error');
      emit(state.copyWith(state: AppointmentStates.reschedulingAppointmentFailed));
    }
  }

  followupAppointment({required String id, required String appointmentDate, required String timeSlot}) async {
    emit(state.copyWith(state: AppointmentStates.followingupAppointment));
    try {
      String? message = await service.followupAppointment(id: id, appointmentDate: appointmentDate, timeSlot: timeSlot);
      emit(state.copyWith(state: AppointmentStates.appointmentFollowedup));
    } catch (error) {
      log('Failed to follow up appointment: $error');
      emit(state.copyWith(state: AppointmentStates.followingupAppointmentFailed));
    }
  }
  ...
  cancellAppointment({required String id}) async {
    ...
    try {
      String? message = await service.cancellAppointment(id: id);
      emit(state.copyWith(state: AppointmentStates.appointmentCancelled));
    } catch (error) {
      log('Failed to cancell appointment: $error');
      emit(state.copyWith(state: AppointmentStates.cancellingAppointmentFailed));
    }
  }
  ...
  Future<void> getCompletedAndRemainingAppointmentCount({required String date}) async {
    emit(state.copyWith(
      state: AppointmentStates.fetchingAppointmentCount,
    ));
    try {
      Map patientsHelped = await service.getCompletedAndRemainingAppointmentCount(date: date);
      emit(state.copyWith(
