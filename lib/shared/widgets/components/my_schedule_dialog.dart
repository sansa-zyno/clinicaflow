...
import 'package:clinica_flow/features/appointment/view/timeslot_gridview.dart';
...
// import 'package:clinica_flow/widgets/section_text.dart';
...
class _MyScheduleDialogState extends State<MyScheduleDialog> with UiInfoMixin {
  ...
  fetchDoctorWithTimeSlots() async {
    List<Map<String, dynamic>>? doctors =
        await AppointmentServices().fetchDoctorsWithTimeSlots();
      ...
      return BlocListener<AppointmentCubit, AppointmentState>(
        listener: (context, state) {
          if (state.state == AppointmentStates.appointmentFollowedup) {
            showSnackbar(
                "Appointment followup completed successfully", context);
            context
                .read<AppointmentCubit>()
                .getAppointmentById(id: widget.appointment.id!)
                .then((value) {
              if (context.read<AppointmentCubit>().state.state ==
                  AppointmentStates.appointmentByIdFetched) {
                context
                    .read<AppointmentCubit>()
            ...
            showSnackbar(
                "Appointment reschedule completed successfully", context);
            context
                .read<AppointmentCubit>()
                .getAppointmentById(id: widget.appointment.id!)
                .then((value) {
                    ...
                    children: [
                      ...
                      Wrap(
                        ...
                        children: List<Widget>.generate(
                            widget.schedule.durations.length, (index) {
                          ...
                          return SelectableContainer(
                            ...
                            selected:
                                duration == widget.schedule.selectedDuration,
                            onTap: () async {
                              final durationsLength =
                                  widget.schedule.durations.length;
                              ...
                              if (index != durationsLength - 1) {
                                switch (duration) {
                                  case 'None':
                                    newAppointmentDateTime = DateTime.parse(
                                ...
                                if (date != null) {
                                  if (widget.schedule.durations.length > 4) {
                                    ...
                                    widget.schedule.durations.insert(0,
                                        '${date.day.toString().padLeft(2, '0')}-${date.month.toString().padLeft(2, '0')}-${date.year}');
                                    newAppointmentDateTime = date;
                                  }
                                  setState(() {
                      ...
                      BlocBuilder<AppointmentCubit, AppointmentState>(
                        ...
                        if (state.state ==
                                AppointmentStates.reschedulingAppointment ||
                            state.state ==
                                AppointmentStates.followingupAppointment) {
                          ...
                          return Row(children: [
                            Expanded(
                              ...
                              textStyle:
                                  const TextStyle(color: AppColors.eerieBlack),
                            )),
