...
class EditAppointmentSettings extends StatefulWidget {
  ...
  const EditAppointmentSettings({super.key, required this.staff});

  @override
  State<EditAppointmentSettings> createState() =>
      _EditAppointmentSettingsState();
}

class _EditAppointmentSettingsState extends State<EditAppointmentSettings>
    with AppBarMixin {
  ...
  void _initializeData() {
    appointmentSlots = widget.staff.availableTimeSlot ?? [];
    ...
    _durationControllers = appointmentSlots
        .map((slot) => TextEditingController(text: slot.duration))
        .toList();
  }
  ...
  void _onStaffStateChanged(BuildContext context, StaffState state) {
    if (state.state == StaffStates.staffCreated) {
      if (widget.staff.id != null) {
        context.read<StaffDetailCubit>().fetchData(widget.staff.id!);
      }
      context.read<StaffCubit>().fetchStaffs();
      for (int i = 0; i < 5; i++) Navigator.pop(context);
  ...
  void _onSave() async {
    ...
    if (clinicId.isNotEmpty) {
      context
          .read<StaffCubit>()
          .updateStaff(widget.staff, context, widget.staff.id!);
    }
  }
